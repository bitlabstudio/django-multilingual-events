...
class EventManager(TranslationManager):
    ...
    def get_upcoming(self, request):
        ...
        qs = qs.filter(
            Q(end_date__gte=timezone.now()) | Q(start_date__gte=timezone.now())
        )
        return qs.distinct()
...
class Event(TranslatableModel):
    ...
    def __str__(self):
        return self.safe_translation_getter(
            "title", "Event on the {0}".format(self.start_date)
        )
    ...
    def get_address(self):
        ...
        full_address = ""
        if self.venue_name:
            full_address += "{0}<br />".format(self.venue_name)
        if self.address_1:
            full_address += "{0}<br />".format(self.address_1)
        if self.address_2:
            full_address += "{0}<br />".format(self.address_2)
        if self.city:
            if self.postal_code:
                full_address += "{0} ".format(self.postal_code)
            full_address += "{0}<br />".format(self.city)
        full_address += "{0}".format(self.country.name)
        return full_address

    def get_alternative_events(self):
        return (
            Event.objects.filter(category=self.category)
            .exclude(pk=self.pk)
            .order_by("start_date")
        )
